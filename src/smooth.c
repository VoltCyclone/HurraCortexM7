// smooth.c — movement injection queue
...
static inline float fast_invsqrt(float x)
{
	...
	float est = v.f;
	est *= (1.5f - 0.5f * x * est * est);
	return est;
}

static inline int32_t ease_asymmetric(int32_t t, float attack_frac,
	float inv_attack_frac, float inv_tail)
{
	float tf = (float)t * (1.0f / (float)SMOOTH_FP_ONE);

	float result;
	if (tf < attack_frac) {
		float n = tf * inv_attack_frac;
		result = n * n * SMOOTH_EASE_ATTACK_DIST;
	} else {
		float n = (tf - attack_frac) * inv_tail;
		float omt = 1.0f - n;
...
static struct {
	...
	float   inv_interval_f;  // 1.0f / interval_us, precomputed
	int32_t inv_frames_lut[SMOOTH_MAX_FRAMES + 1]; // SMOOTH_FP_ONE / n
	bool    humanize;        // humanization toggle (runtime)

	uint32_t rng_a, rng_b, rng_c, rng_counter; // SFC32 frame noise
	...
	uint8_t consec_skips;
	float   rate_bias;
	uint32_t trng_buf[32];       // cached TRNG entropy
	uint8_t  trng_idx;
	uint32_t timing_lfsr;        // xorshift32 fallback
} state;
...
static float compute_fitts_speed_gain(int32_t abs_x_fp, int32_t abs_y_fp)
{
	...
	if (x_lo < 10.0f) {
		float ex = fast_exp2f(x_lo * 1.4426950f);
...
void smooth_init(uint32_t interval_us)
{
	...
	state.inv_interval_f = 1.0f / (float)state.interval_us;

	state.inv_frames_lut[0] = 0; // unused; frames >= SMOOTH_MIN_FRAMES
	for (uint32_t n = 1; n <= SMOOTH_MAX_FRAMES; n++)
		state.inv_frames_lut[n] = SMOOTH_FP_ONE / (int32_t)n;
	...
	*dwt_ctrl |= 1;
	...
	// Personality seed: OCOTP hardware UID + DWT
...
void smooth_process_frame(int16_t *out_x, int16_t *out_y)
{
	...
	while (active) {
		...
		int32_t eased_next = ease_asymmetric(next_progress, attack_frac,
			inv_attack_frac, inv_tail);
		int32_t step_frac  = eased_next - eased_now;

		int32_t remaining_progress = SMOOTH_FP_ONE - eased_now;
		int32_t frame_dx, frame_dy;
		if (remaining_progress > 0) {
			int32_t ratio = fp_div(step_frac, remaining_progress);
			frame_dx = fp_mul(e->x_remaining_fp, ratio);
			frame_dy = fp_mul(e->y_remaining_fp, ratio);
		} else {
...
uint32_t smooth_timing_next(uint32_t base_ldval, bool *out_skip)
{
	*out_skip = false;
	...
	float u1 = timing_rand_uniform();
	float u2 = timing_rand_uniform();
